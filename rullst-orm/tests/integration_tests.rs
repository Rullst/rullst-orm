...
async fn test_schema_and_transaction() {
    ...
    Orm::init(&format!("sqlite:{}?mode=rwc", db_path)).await.unwrap();
    ...
    let mut record = TestRecord {
        ...
        data: Json(MyJsonData { field: "value1".to_string() }),
    };
    ...
    let fetched = TestRecord::query().where_eq("name", "Record1").first_with_tx(&mut tx).await.unwrap().unwrap();
    ...
    assert!(query_after_drop.is_err(), "Query should fail after table is dropped");
